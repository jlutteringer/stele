...
(defn- filter-text-box-base
  ...
  []
  (fn [filter-box? filter-text key-handler open-state-handler]
    [:div.chosen-search
     [:input
      {:type          "text"
       :auto-complete "off"
       ...
       :on-blur       (handler-fn (open-state-handler false))}]]))
...
(defn- dropdown-top-old
  "Render the top part of the dropdown, with the clickable area and the up/down arrow"
  []
  (let [ignore-click (atom false)]
    (fn
      [internal-model choices id-fn label-fn tab-index placeholder dropdown-click key-handler filter-box? open-state-handler]
...
;TODO make this more specific... move to utility package
(defn ratom? [val] (satisfies? IAtom val))
...
(spec/def ::dropdown (spec/keys :req-un [::label ::content ::open-state]))

(def dropdown-schema
  {:name   "Dropdown"
   :fields [[:label :description "This is the description"]
            [:content :description "This is the content"]
            [:open-state :description "Open state atom" :default-fn #(reagent/atom false)]]
   :spec   ::dropdown})
...
;													 (key-handler event)
;													 (when (= (.-which event) 13)  ;; Pressing enter on an anchor also triggers click event, which we don't want
;														 (reset! ignore-click true)))  ;; TODO: Hmmm, have wa look at calling preventDefault (and stopProp?) and removing the ignore-click stuff
;
;					}
;				 [:span
;					(if @internal-model
...
(def dropdown-top
  (schema/component-handler
    (fn [{:keys [label open-state]}]
      [:a.chosen-single.chosen-default
       {:href     "javascript:"                             ;; Required to make this anchor appear in the tab order
        :on-click (handler-fn (swap! open-state not))
        }
       [:span label] [:div [:b]]])))
  ...
  (schema/component-handler
    ...
    (fn [{:keys [label content open-state]}]
      [:div {:class (str "rc-dropdown chosen-container chosen-container-single noselect "
                         (when @open-state "chosen-container-active chosen-with-drop "))}
       [dropdown-top :label label :open-state open-state]
       (when @open-state
         [:div.chosen-drop content])])))

;TODO this should be a utility and needs to be filled out
(def reagent-events
  #{:on-click})

(defn extract-event-map [target-map] (select-keys target-map reagent-events))

(defn merge-event-maps [target-event-map source-event-map]
  (merge-with util/route-fn target-event-map source-event-map))

(defn build-attributes [target-map source-map]
  (merge target-map (merge-event-maps (extract-event-map target-map)
...
(def list-item
  (schema/component-handler
    ...
    (fn [{:keys [label mouse-over-state] :as args}]
      (let [class (when @mouse-over-state "mouseover")]
        [:li
         (build-attributes {:class         (str "active-result group-option " class)
                            :on-mouse-over (handler-fn (reset! mouse-over-state true))
                            :on-mouse-out  (handler-fn (reset! mouse-over-state false))}
                           args)
         label]))))
    ...
    (fn [{:keys [label choices open-state]}]
      [dropdown
       ...
       :content [:ul.chosen-results
                 (let [[group-names group-opt-lists] (choices-with-group-headings choices :group)
                       make-a-choice #(-> [list-item %])
...
(defn build-selection-handler [choice current-selection-atom]
...
(def select
  (schema/component-handler
    ...
    (fn [{:keys [choices open-state current-selection]}]
      [dropdown-list
       :label (:label @current-selection)
       :choices (map
                  #(assoc % :on-click (build-selection-handler % current-selection))
                  choices)
       :open-state open-state])))
...
(def single-dropdown-args-desc
  [{:name :choices :required true :type "vector of choices | atom" :validate-fn vector-of-maps? :description [:span "Each is expected to have an id, label and, optionally, a group, provided by " [:code ":id-fn"] ", " [:code ":label-fn"] " & " [:code ":group-fn"]]}
   ...
   {:name :on-open :required false :default #() :type "boolean -> nil" :validate-fn fn? :description [:span "called when the dropdown is opened or closed. passed true for open, false for closed"]}
...
(defn select-old
  ...
  (let [external-model (reagent/atom (deref-or-value model)) ;; Holds the last known external value of model, to detect external model changes
    ...
    (fn [& {:keys [choices model on-change on-open disabled? filter-box? regex-filter? placeholder width max-height tab-index id-fn label-fn group-fn class style attr]
            :or   {id-fn :id label-fn :label group-fn :group}
            :as   args}]
      {:pre [(validate-args-macro single-dropdown-args-desc args "single-dropdown")]}
      (let [
            on-open (if (nil? on-open) (fn [_] nil) on-open)
            open-state-handler (fn
                                 ([] @drop-showing?)
                                 ([arg]
                                  (if (fn? arg)
                                    (swap! drop-showing? arg)
                                    (reset! drop-showing? arg))
                                  (on-open @drop-showing?)))
            ...
            callback #(do
                       ...
                       (open-state-handler not)             ;; toggle to allow opening dropdown on Enter key
                       (reset! filter-text ""))
            cancel #(do
                     (open-state-handler false)
                     (reset! filter-text "")
                     (reset! internal-model @external-model))
            dropdown-click #(when-not disabled?
                             (open-state-handler not))
            ...
            press-tab (fn []
                        ...
                        true)
            press-up (fn []
                       (if (open-state-handler)             ;; Up arrow
                         (reset! internal-model (move-to-new-choice filtered-choices id-fn @internal-model -1))
                         (open-state-handler true))
                       true)
            press-down (fn []
                         (if (open-state-handler)           ;; Down arrow
                           (reset! internal-model (move-to-new-choice filtered-choices id-fn @internal-model 1))
                           (open-state-handler true))
                         true)
            press-home (fn []
        ...
        [:div
         (merge
           {:class (str "rc-dropdown chosen-container chosen-container-single noselect " (when (open-state-handler) "chosen-container-active chosen-with-drop ") class)
         ...
         [dropdown-top-old internal-model choices id-fn label-fn tab-index placeholder dropdown-click key-handler filter-box? open-state-handler]
         (when (and (open-state-handler) (not disabled?))
           [:div.chosen-drop
            [filter-text-box filter-box? filter-text key-handler open-state-handler]
            [:ul.chosen-results
