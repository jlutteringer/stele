...
(def block
  (butil/component
    (fn [{:keys [title body] :as args}]
      [:div.card-block
       (when (some? title) [:h4.card-title title])
       body])
    ...
    (fn [{:keys [body] :as args}]
      [:div.card
       (map-indexed (fn [i form]
                      (if (and (butil/component-type? form b/image) (= i 0))
                        (butil/make-merged-element form b/image-schema {} {:class ["card-img-top"]})
                        form)) body)])
    :schema card-schema
    :static))
